...
export const SystemOverview = () => {
  const { state, isLive, tankLevel, pumpStatus, totalFlow, avgPressure, togglePump, toggleValve } =
    useSimulationData();
    ...
    <div className="p-6 bg-white rounded-lg shadow">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="p-4 bg-green-50 rounded-lg">
          ...
          <p
            className={`text-2xl font-bold ${pumpStatus === 'ON' ? 'text-green-600' : 'text-gray-600'}`}
          >
            {pumpStatus}
          </p>
        </div>
...
export const TankMonitor = () => {
  const { level, capacity, volume, inletStatus, outletStatus, quality, toggleInlet, toggleOutlet } =
    useTank();
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      ...
      <div className="space-y-2 text-sm">
        <p>
          <strong>Capacity:</strong> {capacity.toLocaleString()} L
        </p>
        <p>
          <strong>Current Volume:</strong> {volume.toLocaleString()} L
        </p>
        <p>
          <strong>Inlet Valve:</strong> {inletStatus}
        </p>
        <p>
          <strong>Outlet Valve:</strong> {outletStatus}
        </p>
      </div>
...
export const PumpMonitor = () => {
  const { status, flowOutput, pressureOutput, runningHours, powerConsumption, motorTemp, toggle } =
    usePump();
...
export const PipelineMonitor = ({ pipelineId }) => {
  ...
  return (
    <div
      className={`p-6 rounded-lg shadow ${hasLeakage ? 'bg-red-50 border-2 border-red-300' : 'bg-white'}`}
    >

...
import { generateMock24hFlow, generateMockTickets, generateResponseTimeData } from '../constants/mockData';
...
export const useIoTSimulation = (active, user) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setData(prev => {
        ...
        const idealFlow = 450;
        const dropPercent = isRunning ? Math.max(0, ((idealFlow - currentFlow) / idealFlow) * 100) : 0;
        const idealEnergy = 12.5; 
        const spikePercent = isRunning ? Math.max(0, ((powerConsumption - idealEnergy) / idealEnergy) * 100) : 0;
        const actualFillRate = 5; 
        const expectedFillRate = 6; 
        const delayPercent = Math.max(0, ((expectedFillRate - actualFillRate)/expectedFillRate)*100);

        const newData = {
          ...prev,
          pumpRunningHours: isRunning ? prev.pumpRunningHours + timeStepHours : prev.pumpRunningHours,
          dailyEnergyKWh: prev.dailyEnergyKWh + energyStep,
          
          pumpMotorTemp: isRunning ? Math.min(90, prev.pumpMotorTemp + 0.5) : Math.max(30, prev.pumpMotorTemp - 0.5),
          pumpFlowRate: Math.max(0, currentFlow),
          pipePressure: Math.max(0, targetPressure + noise(0.2)),
          tankLevel: Math.max(0, Math.min(100, prev.tankLevel + (isRunning && isOpen ? 0.05 : -0.02))),
        ...
        const newAlerts = [];
        if (newData.pipeLeakDropDetected) newAlerts.push({ id: 'leak', type: 'critical', msg: 'Sudden Flow Drop: Possible Leak', category: 'NETWORK' });
        if (newData.pipeBurstDetected) newAlerts.push({ id: 'burst', type: 'critical', msg: 'Sudden Pressure Loss: Burst Pipe', category: 'NETWORK' });
        if (newData.pumpMotorTemp > THRESHOLDS.tempHigh) newAlerts.push({ id: 'temp', type: 'warning', msg: 'Pump Overheating', category: 'PUMP' });
        if (newData.tankOverflow) newAlerts.push({ id: 'overflow', type: 'warning', msg: 'Tank Overflow Warning', category: 'TANK' });
  ...
  const togglePump = () => {
    ...
    setData(prev => ({ 
      ...
      dailyDistributionCycles: newStatus === 'RUNNING' ? prev.dailyDistributionCycles + 1 : prev.dailyDistributionCycles
    }));
  ...
  const toggleValve = () => {
    ...
    setData(prev => ({ ...prev, valveStatus: newStatus, valveLastOpTime: (Math.random()*2 + 0.5).toFixed(1) }));

...
const systemState = {
  ...
  pumpHouse: {
    ...
    powerFactor: 0.85,
    installationDate: Date.now() - 18 * 30 * 24 * 60 * 60 * 1000,
    warrantyPeriodMonths: 24,
    warrantyExpiryDate: Date.now() - 6 * 30 * 24 * 60 * 60 * 1000,
    warrantyStatus: 'EXPIRED',
    warrantyProvider: 'KSB Pumps Ltd.',
    lastMaintenanceDate: Date.now() - 7 * 24 * 60 * 60 * 1000,
    nextMaintenanceDate: Date.now() + 23 * 24 * 60 * 60 * 1000,
    nextServiceDate: Date.now() + 23 * 24 * 60 * 60 * 1000,
    maintenanceIntervalDays: 30,
    operationCycles: 342,
    vibration: 3.2,
    maintenanceHistory: [
    ...
    maintenanceHistory: [
      {
        ...
        notes:
          leakageProb > 10
            ? 'Minor leakage detected, scheduled repair'
...
function applyPumpScheduleLogic() {
  ...
  if (
    ...
    if (forcePumpOff('SCHEDULED_STOP')) {
      addAlert('INFO', 'PUMP_TIMER', `Pump stopped at scheduled time (${stopTime})`);
    }
  }
}
...
function calculateFlowAndPressure() {
  ...
  if (pump.pumpStatus === 'ON') {
    ...
    pump.motorTemperature = clamp(
      pump.motorTemperature + 0.05,
      MIN_MOTOR_TEMPERATURE,
    ...
    if (tank.outletValveStatus === 'OPEN' && numOpen > 0) {
      ...
      systemState.pipelines.forEach((pipeline) => {
        if (pipeline.valveStatus === 'OPEN') {
          ...
          const inletFlow = clamp(
            randomVariation(baseFlowPerPipeline, FLOW_NOISE),
            0,
            MAX_FLOW_LPM
          );
          ...
          if (
            currentInletFlow < 0.5 &&
            currentInletPressure < 0.05 &&
            currentOutletFlow < 0.5 &&
            currentOutletPressure < 0.05
          ) {
    ...
    if (
      systemState.overheadTank.outletValveStatus === 'OPEN' &&
      numOpen > 0 &&
      tankHeadMeters > 0
    ) {
      const area = Math.PI * Math.pow(PIPE_DIAMETER_M / 2, 2);
...
function simulateQualityChanges() {
  ...
  systemState.pipelines.forEach((pipeline) => {
    ...
    const hasActiveFlow =
      pipeline.valveStatus === 'OPEN' && (pipeline.inlet?.flowSensor?.value || 0) > 0;
    if (hasActiveFlow) {
      const leakageFactor = pipeline.leakageProbability / 100;
      ...
      const deviation =
        Math.abs(
          (tankQuality.turbidity - pipeline.outlet.qualitySensor.turbidity) /
            tankQuality.turbidity +
      ...
      if (Math.random() < 0.02) {
        ...
        switch (anomalyType) {
          ...
          case 1: // Chlorine drop
            pipeline.outlet.qualitySensor.chlorine = Math.max(
              0.1,
              pipeline.outlet.qualitySensor.chlorine - 0.2
            );
            addAlert('WARNING', 'QUALITY', `Low chlorine detected in ${pipeline.pipelineName}`);
            break;
          case 2: // Pressure dip
            pipeline.outlet.pressureSensor.value = Math.max(
              0,
              pipeline.outlet.pressureSensor.value - 0.5
            );
  ...
  systemState.pipelines.forEach((p) => {
    if (p.valveStatus === 'OPEN') {
      if (p.pipelineId === leakingId) {
        ...
        p.outlet.pressureSensor.value = Math.max(
          0,
          Math.round((p.inlet.pressureSensor.value - expectedDrop) * 100) / 100
        );
...
function updateSystemMetrics() {
  ...
  const avgDeviation =
    systemState.pipelines.reduce((sum, p) => sum + p.qualityDeviation, 0) /
    systemState.pipelines.length;
...
function addRealtimeEntry() {
  ...
  const entry = {
    ...
    leakageProbability: Math.round(
      systemState.pipelines.reduce((sum, p) => sum + p.leakageProbability, 0) /
        systemState.pipelines.length
    ),
    valveStatus: {

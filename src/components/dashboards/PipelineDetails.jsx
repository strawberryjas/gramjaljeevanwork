    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl overflow-hidden shadow-2xl border border-gray-700/50">
        <div className="bg-gradient-to-r from-slate-900 to-slate-950 p-6 border-b border-gray-700/50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Underground Distribution Pipeline • Length: {(pipeline.length || 450).toFixed(0)}m</p>
            </div>
        ...
        <div className="p-8 relative">
          {/* Ambient Glow Effect */}
          {isOpen && !hasLeakage && (
            <div className="absolute inset-0 bg-gradient-radial from-cyan-500/5 via-blue-500/5 to-transparent animate-pulse pointer-events-none" style={{animationDuration: '3s'}}></div>
          )}
          {hasLeakage && isOpen && (
            <div className="absolute inset-0 bg-gradient-radial from-red-500/10 via-orange-500/5 to-transparent animate-pulse pointer-events-none" style={{animationDuration: '2s'}}></div>
          )}
          ...
          <div className="relative flex items-center justify-center">
            <svg viewBox="0 0 1000 400" className="w-full h-auto" style={{ maxHeight: '400px' }}>
              ...
              <line x1="0" y1="280" x2="1000" y2="280" stroke="#27272a" strokeWidth="2" strokeDasharray="10,5"/>

              {/* Inlet Chamber/Connection */}
              <g transform="translate(50, 180)">
                <rect x="-30" y="-40" width="60" height="100" fill="#27272a" stroke="#52525b" strokeWidth="3" rx="4" filter="url(#pipeDropShadow)"/>
                <rect x="-25" y="-35" width="50" height="90" fill="#18181b" stroke="#3f3f46" strokeWidth="2" rx="3"/>
                ...
                {(() => {
                  ...
                  return (inletFlow > 0.5 || inletPressure > 0.05) && (
                    <g opacity={flowOpacity}>
                      {/* Water fill with gradient */}
                      <defs>
                        <linearGradient id="inletWaterGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                          <stop offset="0%" stopColor={inletWaterColor.bright} stopOpacity="0.6"/>
                          <stop offset="100%" stopColor={inletWaterColor.dark} stopOpacity="0.8"/>
                        </linearGradient>
                      </defs>
                      
                      {/* Main water body */}
                      <rect x="-22" y={waterY} width="44" height={waterHeight} fill="url(#inletWaterGrad)" rx="2"/>
                      
                      {/* Animated water surface with ripples */}
                      <path d={`M -22,${waterY} Q -11,${waterY-2} 0,${waterY} T 22,${waterY}`} 
                            fill={inletWaterColor.light} opacity="0.5">
                      ...
                      {[0, 1, 2].map((i) => (
                        <circle key={i} cx={-12 + i * 12} cy={waterY + 10} r="1.5" fill="#e0f2fe" opacity="0.7">
                          <animate attributeName="cy" from={waterY + waterHeight - 5} to={waterY} 
                ...
                <text x="0" y="-50" fontSize="11" fill="#71717a" textAnchor="middle" fontWeight="bold">INLET</text>
                <text x="0" y="85" fontSize="14" fill="#06b6d4" textAnchor="middle" fontWeight="bold">{inletFlow.toFixed(1)}</text>
                <text x="0" y="100" fontSize="9" fill="#71717a" textAnchor="middle">L/min</text>
                <text x="0" y="115" fontSize="9" fill="#10b981" textAnchor="middle">{inletPressure.toFixed(2)} Bar</text>
              </g>
              ...
              <g transform="translate(950, 180)">
                ...
                {(() => {
                  ...
                  return (outletFlow > 0.5 || outletPressure > 0.05) && (
                    <g opacity={flowOpacity}>
                      {/* Water fill with gradient */}
                      <defs>
                        <linearGradient id="outletWaterGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                          <stop offset="0%" stopColor={outletWaterColor.bright} stopOpacity="0.6"/>
                          <stop offset="100%" stopColor={outletWaterColor.dark} stopOpacity="0.8"/>
                        </linearGradient>
                      </defs>
                      
                      {/* Main water body */}
                      <rect x="-22" y={waterY} width="44" height={waterHeight} fill="url(#outletWaterGrad)" rx="2"/>
                      
                      {/* Animated water surface with ripples */}
                      <path d={`M -22,${waterY} Q -11,${waterY-2} 0,${waterY} T 22,${waterY}`} 
                            fill={outletWaterColor.light} opacity="0.5">
                        <animate attributeName="d" 
                          values={`M -22,${waterY} Q -11,${waterY-2} 0,${waterY} T 22,${waterY};
                                  M -22,${waterY} Q -11,${waterY+2} 0,${waterY} T 22,${waterY};
                                  M -22,${waterY} Q -11,${waterY-2} 0,${waterY} T 22,${waterY}`}
                ...
                <text x="0" y="-50" fontSize="11" fill="#71717a" textAnchor="middle" fontWeight="bold">OUTLET</text>
                <text x="0" y="85" fontSize="14" fill="#06b6d4" textAnchor="middle" fontWeight="bold">{outletFlow.toFixed(1)}</text>
                <text x="0" y="100" fontSize="9" fill="#71717a" textAnchor="middle">L/min</text>
                <text x="0" y="115" fontSize="9" fill="#10b981" textAnchor="middle">{outletPressure.toFixed(2)} Bar</text>
              </g>
              ...
              <g filter="url(#pipeDropShadow)">
                ...
                <rect x="110" y="160" width="340" height="60" fill={isOpen ? "url(#pipeMetalGradient)" : "url(#pipeMetalGradientClosed)"} rx="6"/>
                ...
                {[150, 250, 350].map((x, i) => (
                  <g key={i}>
                    ...
                    {/* Coupling Bolts */}
                    <circle cx={x+8} cy="165" r="2.5" fill="#27272a" stroke="#71717a" strokeWidth="0.5"/>
                    <circle cx={x+8} cy="215" r="2.5" fill="#27272a" stroke="#71717a" strokeWidth="0.5"/>
                  </g>
                ))}
                ...
                {(inletFlow > 0.5 || inletPressure > 0.05) && !hasLeakage && (
                  <g key={`section1-normal-${pipelineId}`} clipPath="url(#pipeClip1)">
                    ...
                    {/* Base Water Layer */}
                    <rect x="115" y="170" width="330" height="40" fill={inletWaterColor.base} opacity={0.7 * flowOpacity}/>
                    
                    {/* Animated Flowing Layer */}
                    <rect x="115" y="170" width="330" height="40" fill="url(#flowingWater1)" opacity={0.8 * flowOpacity}/>
                    ...
                    {[...Array(12)].map((_, i) => {
                      ...
                      return (
                        ...
                        >
                          <animate attributeName="cx" from="120" to="440" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                          <animate attributeName="opacity" values="0.3;0.7;0.3" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                          <animate attributeName="ry" values={`${size * 0.8};${size * 1.2};${size * 0.8}`} dur="1s" repeatCount="indefinite"/>
                        </ellipse>
                      );
                    })}
                    
                    {/* Surface Ripples */}
                    <path d="M 115,172 Q 160,168 205,172 T 295,172 T 385,172 T 445,172" stroke={inletWaterColor.bright} strokeWidth="1" fill="none" opacity="0.4">
                      <animate attributeName="d" 
                ...
                {(inletFlow > 0.5 || inletPressure > 0.05) && hasLeakage && (
                  <g key={`section1-leak-${pipelineId}`} clipPath="url(#pipeClip1Leak)">
                    ...
                    <rect x="115" y="180" width="330" height="28" fill={inletWaterColor.base} opacity="0.6"/>
                    
                    {/* Slower water movement due to leak (not contamination) */}
                    <rect x="115" y="180" width="330" height="28" fill={inletWaterColor.light} opacity="0.3">
                      <animate attributeName="x" from="115" to="85" dur="4s" repeatCount="indefinite"/>
                    ...
                    {[...Array(6)].map((_, i) => {
                      ...
                      return (
                        ...
                        >
                          <animate attributeName="cx" from="120" to="440" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                          <animate attributeName="cy" values={`${yPos};${yPos+2};${yPos}`} dur="2.5s" repeatCount="indefinite"/>
                ...
                {/* No Flow Indicator */}
                {!isOpen && (
                  <text x="280" y="195" fontSize="14" fill="#71717a" textAnchor="middle" fontWeight="bold">NO FLOW</text>
                )}
              </g>
              ...
              >
                {/* Valve Body */}
                <rect x="-45" y="-60" width="90" height="80" fill="url(#valveGradient)" stroke="#71717a" strokeWidth="3" rx="6"/>
                <rect x="-40" y="-55" width="80" height="70" fill="#3f3f46" stroke="#52525b" strokeWidth="2" rx="4"/>
                ...
                {/* Valve Label */}
                <rect x="-35" y="25" width="70" height="22" fill="#fcd34d" stroke="#f59e0b" strokeWidth="2" rx="3"/>
                <text x="0" y="40" fontSize="10" fill="#78350f" textAnchor="middle" fontWeight="bold">MAIN VALVE</text>
              ...
              <g filter="url(#pipeDropShadow)">
                ...
                {(() => {
                  const hasBlockage = (inletPressure - outletPressure) > 1.2 && isOpen && flowLoss > 15;
                  
                  return (
                    <>
                      {/* Outer Pipe Body - Concrete/PVC Style for Rural India */}
                      <rect x="550" y="160" width="340" height="60" fill={isOpen ? "url(#pipeMetalGradient)" : "url(#pipeMetalGradientClosed)"} rx="6"/>
                      <rect x="555" y="165" width="330" height="50" fill="#52525b" rx="4"/>
                      
                      {/* Weathered/Aged appearance */}
                      <rect x="555" y="165" width="330" height="8" fill="#71717a" opacity="0.3" rx="4"/>
                      <rect x="555" y="207" width="330" height="8" fill="#27272a" opacity="0.2" rx="4"/>
                      ...
                      {hasBlockage && (
                        <g>
                          ...
                          <rect x="560" y="172" width="120" height="36" fill="#ef4444" opacity="0.2">
                            <animate attributeName="opacity" values="0.15;0.3;0.15" dur="1.5s" repeatCount="indefinite"/>
                          </rect>
                          
                          {/* Turbulent particles before blockage */}
                          {[0, 1, 2].map((i) => (
                            <circle key={i} cx="620" cy={180 + (i * 10)} r="2.5" fill="#f87171" opacity="0.6">
                              <animate attributeName="cx" values="560;680;560" dur="2s" begin={`${i * 0.4}s`} repeatCount="indefinite"/>
                              <animate attributeName="opacity" values="0.4;0.8;0.4" dur="2s" begin={`${i * 0.4}s`} repeatCount="indefinite"/>
                            </circle>
                          ))}
                          
                          {/* Blockage Warning */}
                          <g transform="translate(700, 250)">
                            <rect x="-45" y="0" width="90" height="28" fill="#dc2626" stroke="#991b1b" strokeWidth="2" rx="4"/>
                            <text x="0" y="18" fontSize="10" fill="#fff" textAnchor="middle" fontWeight="bold">⚠ BLOCKAGE</text>
                          </g>
                        </g>
                      )}

                      {/* Normal Water Flow (Section 2) - Realistic Flowing Water with Quality-Based Color */}
                      {(outletFlow > 0.5 || outletPressure > 0.05) && !hasBlockage && !hasLeakage && (
                        <g key={`section2-normal-${pipelineId}`} clipPath="url(#pipeClip2)">
                          <defs>
                            ...
                            <linearGradient id="flowingWater2" x1="0%" y1="0%" x2="100%" y2="0%">
                              <stop offset="0%" stopColor={outletWaterColor.dark} stopOpacity="0.9">
                                <animate attributeName="offset" values="-1;1" dur="2s" repeatCount="indefinite"/>
                              </stop>
                              <stop offset="25%" stopColor={outletWaterColor.base} stopOpacity="0.95">
                                <animate attributeName="offset" values="-0.75;1.25" dur="2s" repeatCount="indefinite"/>
                              </stop>
                              <stop offset="50%" stopColor={outletWaterColor.light} stopOpacity="1">
                                <animate attributeName="offset" values="-0.5;1.5" dur="2s" repeatCount="indefinite"/>
                              </stop>
                              <stop offset="75%" stopColor={outletWaterColor.bright} stopOpacity="0.95">
                                <animate attributeName="offset" values="-0.25;1.75" dur="2s" repeatCount="indefinite"/>
                              </stop>
                              <stop offset="100%" stopColor={outletWaterColor.dark} stopOpacity="0.9">
                                <animate attributeName="offset" values="0;2" dur="2s" repeatCount="indefinite"/>
                              </stop>
                            </linearGradient>
                          </defs>
                          ...
                          {/* Base Water Layer */}
                          <rect x="555" y="170" width="330" height="40" fill={outletWaterColor.base} opacity={0.7 * flowOpacity}/>
                          
                          {/* Animated Flowing Layer */}
                          <rect x="555" y="170" width="330" height="40" fill="url(#flowingWater2)" opacity={0.8 * flowOpacity}/>
                          ...
                          {[...Array(12)].map((_, i) => {
                            ...
                            return (
                              <ellipse 
                                ...
                                opacity={0.6 * flowOpacity}
                              >
                                <animate attributeName="cx" from="560" to="880" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                                <animate attributeName="opacity" values="0.3;0.7;0.3" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                                <animate attributeName="ry" values={`${size * 0.8};${size * 1.2};${size * 0.8}`} dur="1s" repeatCount="indefinite"/>
                              </ellipse>
                            );
                          ...
                          {/* Surface Ripples */}
                          <path d="M 555,172 Q 600,168 645,172 T 735,172 T 825,172 T 885,172" stroke={outletWaterColor.bright} strokeWidth="1" fill="none" opacity="0.4">
                            <animate attributeName="d" 
                      ...
                      {(outletFlow > 0.5 || outletPressure > 0.05) && !hasBlockage && hasLeakage && (
                        <g key={`section2-leak-${pipelineId}`} clipPath="url(#pipeClip2Leak)">
                          ...
                          <rect x="555" y="180" width="330" height="28" fill={outletWaterColor.base} opacity="0.6"/>
                          
                          {/* Slower water movement due to leak */}
                          <rect x="555" y="180" width="330" height="28" fill={outletWaterColor.light} opacity="0.3">
                            <animate attributeName="x" from="555" to="525" dur="4s" repeatCount="indefinite"/>
                            <animate attributeName="opacity" values="0.2;0.4;0.2" dur="3s" repeatCount="indefinite"/>
                          </rect>
                      ...
                      {hasBlockage && (
                        <g clipPath="url(#pipeClip2Blocked)">
                          ...
                          <rect x="720" y="175" width="165" height="30" fill="url(#waterFlowGradient)" opacity="0.4">
                            <animate attributeName="opacity" values="0.3;0.5;0.3" dur="2.5s" repeatCount="indefinite"/>
                          </rect>
                          
                          {[0, 1].map((i) => (
                            <circle key={i} cx="730" cy={182 + (i * 12)} r="2" fill="#e0f2fe" opacity="0.5">
                              <animate attributeName="cx" from="730" to="880" dur="5s" begin={`${i * 1}s`} repeatCount="indefinite"/>
                            </circle>
                          ))}
                        </g>
                      )}
              ...
              {hasLeakage && (inletFlow > 0.5 || inletPressure > 0.05) && (
                <g key={`leak-${pipelineId}-${hasLeakage}`} transform="translate(400, 220)" opacity={flowOpacity}>
                  {/* Crack/Rupture Point with Pressure */}
                  <path d="M -6,-4 L 6,4 M -6,4 L 6,-4 M 0,-6 L 0,6" stroke="#dc2626" strokeWidth="3" strokeLinecap="round"/>
                  ...
                  <g>
                    ...
                    <path d="M 0,0 Q -15,15 -25,35 L -30,45" stroke={inletWaterColor.light} strokeWidth="4" fill="none" opacity="0.7">
                      <animate attributeName="opacity" values="0.6;0.8;0.6" dur="0.3s" repeatCount="indefinite"/>
                    </path>
                    <path d="M 0,0 Q -8,12 -12,28 L -15,38" stroke={inletWaterColor.bright} strokeWidth="2.5" fill="none" opacity="0.8">
                      <animate attributeName="opacity" values="0.7;1;0.7" dur="0.3s" repeatCount="indefinite"/>
                    </path>
                    
                    {/* Side spray jets */}
                    <path d="M 0,0 Q 10,8 15,20 L 18,30" stroke={inletWaterColor.light} strokeWidth="3" fill="none" opacity="0.6">
                      <animate attributeName="opacity" values="0.5;0.7;0.5" dur="0.4s" repeatCount="indefinite"/>
                    </path>
                    <path d="M 0,0 Q -25,5 -35,18 L -42,28" stroke={inletWaterColor.base} strokeWidth="2.5" fill="none" opacity="0.5">
                      <animate attributeName="opacity" values="0.4;0.6;0.4" dur="0.35s" repeatCount="indefinite"/>
                    </path>
                  </g>
                  ...
                  {[...Array(20)].map((_, i) => {
                    ...
                    return (
                      <circle key={i} cx="0" cy="0" r={size} fill={inletWaterColor.bright} opacity="0.7">
                        <animate attributeName="cx" from="0" to={xEnd} dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                        <animate attributeName="cy" from="0" to={yEnd} dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                        <animate attributeName="opacity" from="0.7" to="0" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                        <animate attributeName="r" from={size} to={size * 0.3} dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                      </circle>
                    );
                  })}
                  ...
                  {[...Array(8)].map((_, i) => {
                    ...
                    return (
                      <ellipse key={`drop-${i}`} cx={xPos} cy={yStart} rx="2" ry="3" fill={inletWaterColor.base} opacity="0.6">
                        <animate attributeName="cy" from={yStart} to={yEnd} dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                        <animate attributeName="opacity" from="0.6" to="0" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                        <animate attributeName="ry" values="3;4;2" dur={`${duration}s`} begin={`${delay}s`} repeatCount="indefinite"/>
                      </ellipse>
                    );
                  })}
                  ...
                  <g transform="translate(0, 75)" filter="url(#glowEffect)">
                    ...
                    <text x="0" y="7" fontSize="12" fill="#dc2626" textAnchor="middle" fontWeight="bold">!</text>
                    <text x="0" y="35" fontSize="10" fill="#dc2626" textAnchor="middle" fontWeight="bold">PRESSURIZED LEAK</text>
              ...
              {(inletFlow > 1 || outletFlow > 1) && (
                <>
                  <g transform="translate(280, 145)">
                    ...
                    <text x="20" y="12" fontSize="9" fill="#71717a" fontWeight="bold">FLOW</text>
                  </g>

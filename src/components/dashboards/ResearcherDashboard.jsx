...
export const ResearcherDashboard = ({ sensors = {}, systemState, history = [] }) => {
  ...
  const realtimeHistory =
    (systemState?.realtimeHistory && systemState.realtimeHistory.length > 0
  ...
  const analyticsStats = [
    {
      label: 'Data Points',
      value: (realtimeHistory.length || 0).toLocaleString(),
      icon: Database,
      color: 'blue',
      trend: `${pipelines.length} feeds`,
    },
    {
      label: 'Avg Flow Rate',
      value: `${(metrics.totalFlowRate || pump.pumpFlowOutput || 0).toFixed(1)} L/min`,
      icon: Activity,
      color: 'emerald',
      trend: `${openPipelines} open`,
    },
    {
      label: 'Efficiency',
      value: `${(metrics.systemEfficiency || pump.pumpEfficiency || 0).toFixed(1)}%`,
      icon: TrendingUp,
      color: 'green',
      trend: `${metrics.totalLeakage || 0} L loss`,
    },
    {
      label: 'Pipelines Flagged',
      value: `${qualityAlerts}`,
      icon: FileText,
      color: 'purple',
      trend: `${leakWatch} leak watch`,
    },
  ];

  const trendDataSource = realtimeHistory
    .slice(0, 30)
    .map((entry) => ({
      day: entry.timestamp
      ...
      quality: entry.turbidity
        ? Math.max(0, 100 - entry.turbidity * 10)
        : Math.max(0, 100 - (metrics.averageQualityDeviation || 0)),
    }))
    .reverse();

  const mockTrendData =
    trendDataSource.length > 0
      ? trendDataSource
  ...
  const waterQualityData = [
    {
      ...
      min: 6.5,
      max: 8.5,
      status: currentPH >= 6.5 && currentPH <= 8.5 ? 'Good' : 'Out of Range',
    },
    {
      parameter: 'Turbidity',
      ...
      min: 0,
      max: 5,
      status: currentTurbidity <= 5 ? 'Excellent' : 'High',
    },
    {
      parameter: 'Chlorine',
      value: currentChlorine.toFixed(2),
      min: 0.2,
      max: 1.0,
      status: currentChlorine >= 0.2 && currentChlorine <= 1 ? 'Good' : 'Adjust',
    },
    {
      parameter: 'TDS',
      value: currentTDS.toFixed(0),
      min: 0,
      max: 500,
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {analyticsStats.map((stat, idx) => (
              <div
                key={idx}
                className={`bg-white rounded-lg shadow-md p-6 border-l-4 border-${stat.color}-600`}
              >
